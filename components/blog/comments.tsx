...
export function Comments() {
	...
	useEffect(() => {
		...
		if (isDev && isUsingProxy) {
			console.log(
				"⏭️ Skipping Cusdis in development (proxy only works in production)"
			);
			return;
		}
		...
		window.CUSDIS_LOCALE = {
			...
			comment_has_been_sent: "አስተያየትዎ ተልኳል። እባክዎን እስኪጸድቅ ይጠብቁ።",
		};
		...
		const loadScript = (attempt = 1) => {
			...
			const timeoutId = setTimeout(() => {
				if (!scriptLoaded) {
					console.warn(
						`⏱️ Script loading timeout (attempt ${attempt}/3)`
					);
			...
			script.onerror = (error) => {
				...
				console.error(
					`❌ Failed to load Cusdis SDK (attempt ${attempt}/3)`
				);
				...
				if (attempt < 3) {
					...
					// All retries failed
					console.error(
						"❌ All retry attempts failed. Checking Railway deployment status..."
					);
					setLoadError(
	...
	useEffect(() => {
		if (!isConfigured || !scriptLoaded || !resolvedTheme || !ref.current)
			return;
		...
		const initializeCusdis = () => {
			...
			const checkAndRender = () => {
				...
				if (window.CUSDIS) {
					...
					setTimeout(() => {
						if (window.CUSDIS && cusdisDiv) {
							try {
								...
								setLoadError(
									"Failed to initialize Cusdis. Please refresh the page."
								);
							}
						}
					}, 100);
					...
					console.log(
						`Waiting for CUSDIS object... (attempt ${attempts}/${maxAttempts})`
					);
					setTimeout(checkAndRender, 100);
				} else {
					console.error(
						"CUSDIS object not available after max attempts"
					);
					setLoadError(
						"Cusdis initialization timed out. Please refresh the page."
					);
				}
			};
						...
						<div className="mb-4 p-4 bg-destructive/10 text-destructive rounded-lg">
							...
							<div className="mt-3 p-3 bg-background rounded text-xs">
								...
								<ol className="list-decimal list-inside space-y-1 text-muted-foreground">
									...
									<li>
										Verify the deployment logs in Railway
									...
									<li>
										Try redeploying your Cusdis service on
										Railway
									</li>
									<li>
								...
								<p className="mt-2 text-xs text-amber-600">
									💡 Alternative: Use Cusdis Cloud at{" "}
									<a
					...
					{!isConfigured ? (
						<div className="text-center py-8">
							...
							<p className="text-sm text-muted-foreground mb-4">
								To enable comments, you need to configure Cusdis
								with your App ID.
							</p>
							<div className="space-y-2 text-sm text-left max-w-2xl mx-auto bg-muted/50 p-4 rounded-lg">
								...
								<ol className="list-decimal list-inside space-y-1 text-muted-foreground">
									...
									<li>
										Create a website and get your App ID
									</li>
									<li>
										Add the following to your environment
										variables (Railway or{" "}
